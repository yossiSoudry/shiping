  ...
  return (
    <div className="py-20 bg-gradient-to-br from-gray-50 to-white">
      <div className="max-w-7xl mx-auto px-4">
        <h2 className="text-4xl font-bold text-center text-blue-900/90 mb-12">
          השירותים שלנו
        </h2>
        <div className="grid md:grid-cols-3 gap-8">
          {services.map((service, index) => (
            ...
            >
              <div className="w-16 h-16 bg-gradient-to-br from-orange-300/90 to-orange-500/90 rounded-full flex items-center justify-center mb-6">
                <service.icon className="w-8 h-8 text-white" />
              </div>
              <h3 className="text-2xl font-bold text-blue-900/90 mb-4">{service.title}</h3>
              <p className="text-gray-600 mb-6">{service.description}</p>
              <Link href={service.link}>
                <span className="text-orange-500/90 hover:text-orange-600 font-medium inline-flex items-center">
                  למידע נוסף ←
                </span>
              </Link>
            </motion.div>
